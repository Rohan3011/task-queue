...
// Mock TaskHandler that just marks tasks as completed.
type MockTaskHandler struct{}

func (h *MockTaskHandler) Handle(t *task.Task) error {
...
func TestTaskQueueProcessing(t *testing.T) {
	...
	taskQueue := NewInMemoryQueue()

	// Register a mock handler for task processing
	task.RegisterHandler("mock", func() task.TaskHandler {
